...
def cadastro_prefeito_view(): # Renomeado para prefeito
    ...
    def cadastrar_prefeito(e): # Renomeado para prefeito
        """Função para cadastrar o prefeito no banco de dados."""
        ...
        try:
            ...
            if not (10000 <= numero_int <= 99999): # 5 dígitos para prefeito
                mensagem.value = "O número do prefeito deve ser um inteiro de 5 dígitos (ex: 12345)."
                e.page.update()
                return
        except ValueError:
            mensagem.value = "O número do prefeito deve ser um valor numérico válido."
            e.page.update()
            return
        ...
        try:
            ...
            cursor.execute("SELECT * FROM Prefeito WHERE id_número_prefeito = ?", (numero_val,)) # Tabela Prefeito
            if cursor.fetchone():
                mensagem.value = "Número já cadastrado para um prefeito!" # Mensagem para prefeito
            else:
                cursor.execute("""
                    INSERT INTO Prefeito (id_número_prefeito, nome, foto, id_partido, descrição)
                    VALUES (?, ?, ?, ?, ?)
                """, (numero_val, nome_val, foto_data, id_partido, descricao_val)) # Tabela Prefeito e colunas
                conn.commit()
            ...
            if "UNIQUE constraint failed: Prefeito.id_número_prefeito" in str(ex): # Tabela Prefeito
                mensagem.value = "Erro: O número do prefeito já está em uso!" # Mensagem para prefeito
            else:
    ...
    return ft.View(
        route="/cadastro_prefeito", # Rota para prefeito
        bgcolor="#B0C4DE",
        controls=[
            file_picker, 
                                    ...
                                    controls=[
                                        ...
                                        ft.Container(
                                            ...
                                            content=ft.Column(
                                                controls=[
                                                    ft.Container(
                                                        content=ft.Text("Prefeito", size=70, weight="bold", color="black"), # Título para prefeito
                                                        alignment=ft.alignment.center_left,
                                                        margin=ft.Margin(left=0, top=0, right=0, bottom=10),
                                                    ),
                                                    styled_textfield(nome, "Ex: João da Silva", "Nome:"),
                                                    styled_textfield(numero, "Ex: 12345", "Número:"), # Exemplo de número para prefeito
                                                    ...
                                                    ft.Row(
                                                        controls=[
                                                            ft.Container(
                                                                ...
                                                                border=ft.border.all(1, "black"),
                                                                ink=True,
                                                                on_click=cadastrar_prefeito, # Função para prefeito
                                                                shadow=ft.BoxShadow(
